  ...
  ai_extraction: {
    ...
    validateConfig: (config) => {
      ...
      for (const f of fields) {
        ...
        // JsonShape is one level deep, so Gemini would reject an ARRAY with no `items` as a
        // non-retryable 400 mid-run. Refused here instead: caught at save, not at execution.
